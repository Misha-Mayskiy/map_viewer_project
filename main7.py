...
class MapViewerApp(QWidget):
    def __init__(self):
        ...
        self.reset_button = QPushButton("Сброс", self)  # Added Reset button
    ...
    def initUI(self):
        self.setWindowTitle('Карта v0.7')
        ...
        search_layout.addWidget(self.reset_button)  # Added to layout
        ...
        self.reset_button.clicked.connect(self.reset_search_result)  # Connect reset button
    ...
    def reset_search_result(self):
        """Clears the search marker and reloads the map."""
        if self.marker_coords:
            print("Сброс результата поиска (удаление метки).")
            self.marker_coords = None
            self.search_input.clear()
            self.load_map()  # Reload map without the marker
        else:
    ...
    def geocode_and_update_map(self, address_to_find):
        ...
        try:
            ...
            if not feature_member:
                ...
                self.reset_search_result()  # Use reset function to clear marker
                return
